...
```{r}
savePath = system.file("extdata", "rezEx.Rdata", package = "rezonateR")
...
After editing the CSV in a spreadsheet program, let's import it back using rez_read_csv. The origDF argument tells rezonateR to look in the original rezrDF that produced the CSV, and determine the data types accordingly:

```{r}
changeDF = rez_read_csv("refexpr.csv", origDF = myRez$trackDF$refexpr)
```
...
myRez$trackDF$refexpr = myRez$trackDF$refexpr %>% rez_left_join(myRez$chunkDF$verb %>% select(id, word, treeEntry), by = c(treeParent = "treeEntry"), suffix = c("", "_verb")) %>% rename(verbID = id_verb, verbWord = word_verb)#This step will be doable with addFieldForeign verb soon.
