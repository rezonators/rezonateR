...
## Research question: Choice of referential expressions in discourse
...
This time, `noCompetitors` is no longer a good predictor. The coefficient of `gapUnitsNew` is still in the 'wrong' direction, since a larger `gapUnitsNew` still leads to a smaller referential expression, but the effect is not significant any more. On the other hand, the predictor `noPrevMentionsin10` remains significant and in the expected direction: The more the number of previous mentions, the shorter the form.
...
As we can see, subjects are likely to be far shorter than non-subjects. The effects of the other factors are almost gone in this model, except `noPrevMentionsIn10`, which remains marginally significant.
...
Let's see if we can improve upon the model in any more ways. The first thing we could do is to split up `noPrevMentionsIn10`. It is often proposed in the literature that subjects are more accessible than other types of arguments, and when the previous mention is a subject, the expression will subsequently be more accessible as well (e.g. Arnold 2008). So we could split up previous mentions into previous subjects and previous non-subjects to see whether they have different effects.
...
As we can now see, non-subject previous mentions have no effect on referential expression length at all, although the effect for subject previous mentions nears significance, and the estimate is in the expected direction (the more previous mentions, the shorter the form).
...
![Figure 18; A screenshot of the .csv file in Excel.](img/sample-excel.PNG){width="100%"}
...
Whether the current mention is a subject continues to turn out to be a good predictor, and the number of previous subject mentions is marginally significant. The rest of the predictors are not significant, though some of them (particularly the number of competitors and lookback distance i.e. `gapUnits`) may hold some promise on a larger dataset. Grammatical number seems to be hopeless, and probably not worth pursuing.

## A post-mortem: Where did all the effects go?

It may be disappointing that only two predictors, number of previous subject mentions and the subject/non-subject contrast, were significant. Even though we know from previous studies that previous mentions are generally important in determining the choice of referential form, only the number of preceding subject mentions turned out to be significant, and even then only marginally.
