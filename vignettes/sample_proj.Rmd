...
Here is how the whole excerpt we have shown at the beginning would look like with all the coreference chains marked (including the first, truncated referential expression *the* in line 365):
...
-   `orderInTrail`: The position of a mention in its coreference chain. That is, is it the first mention of somunething, or the second, third, etc.?
...
Each unit consists of a series of tokens separated by spaces. In this file, most of the tokens are English words, like *usually*, *I* and *do* in line 7 of the screenshot. Also considered tokens are contractions like *'nt* for *not* on lines 7 and 9 or *'s* for *is* on lines 10 and 12, which are considered separate tokens in this project even though they are not separated by whitespace from the previous token in English writing conventions.
...
![Figure 13: Some examples of 'blank chunks', indicating verbal complexes](img/sample_verbs.PNG){width="100%"}
...
![Figure 14: An example of a particle verb where the particle is separated from the verb by an object](images/paste-5DE5A9C5.png){width="430"}
...
![Figure 15: The navigation pane showing two chunk properties, `chunkType` and `largerChunk`](images/paste-F4175735.png)
...
-   **Which fields of a token contains text to be concatenated:** When you are browsing data frames containing data structures above the level of a token, such as `chunkDF`or`unitDF`, you want to be able to see the text. For example, take the following line:
    ...
    ![Figure 16: A unit containing the chunk \*\'m also going to suggest\*](images/paste-5FB50E2C.png){width="444"}
...
![Figure 17: Units and tokens.](img/sample_convo.PNG){alt="Figure 7: Units and tokens." width="74%"}
...
![Figure 18: A trail containing four mentions of the action of staying up late, spanning units 2-6.](images/paste-224A955C.png){width="598"}
...
![Figure 19: A trail containing five mentions of the action of staying up late, spanning units 2-8.](images/paste-E52A7A24.png){width="586"}
...
![Figure 20: Competing mentions to the *it*, referring to the action of staying up late, on line 8.](images/paste-E203783D.png){width="512"}
...
![Figure 21: Coreference chain for the action of staying up late](images/paste-3973BFDF.png){alt="Figure 11: Coreference chain for the action of staying up late" width="614"}
...
A final variable we could add to the model is the sequential position of a track within a trail. We might expect that the second mention of a referent, for example, might be longer than a fifth mention. We can get this variable using `getPosInChain()`:

```{r}
rez007$trackDF$default = rez007$trackDF$default %>%
  rez_mutate(orderInTrail = getPosInChain(tokenOrder = docWordSeqLast))
rez007$trackDF$default %>% select(id, name, text, orderInTrail) %>% head
```
...
rez007$trackDF$default %>%
  filter(gapWords != "N/A", charCount > 0) %>%
  ggplot(aes(x = orderInTrail, y = charCount)) + geom_count()
```

It does seem that larger character counts are much rare as the `orderInTrail` value gets larger. Let's put the variable inside our model and see what happens:

```{r}
pass2_model5 = rez007$trackDF$default %>%
  filter(gapWords != "N/A", charCount > 0) %>%
  mutate(gapUnits = as.numeric(gapUnits)) %>%
  lm(charCount ~ noCompetitors + noPrevMentionsIn10 + gapWordsNew + orderInTrail, .)
summary(pass2_model5)
```

It turns out that `orderInTrail` is a significant variable, and in the expected direction: The farther along we are in a trail, the shorter the form of the referential expression.
...
![Figure 22: A case of cataphoric *it* creating an exception to the general rule that shorter `gapUnits` is associated with shorter length.](images/paste-AA63E6C5.png){width="437"}
...
![Figure 23: A case where three separates should in reality be treated as a single chunk.](images/paste-F5ED0106.png){width="430"}
...
![Figure 24: An example of a tree in our file.](img/sample_tree.PNG){width="100%"}
...
-   `mergeChunksWithTree()`: If there is a treeEntry that corresponds to the concatenation of multiple chunks, then we merge those chunks. For example, in the screenshots below, *(H) \<0\> going to (...) our parents ,* and *and (...) \<0\> complaining about one another* are in separate chunks. However, they correspond to one single tree entry, as we can see here.
    ...
    ![Figure 25: An example where two chunks that should belong together can be merged using the fact that they belong to the same tree entry.](images/paste-0C261A8B.png){width="483"}
...
-   `mergeChunksWithIDs()`: We merge chunks by assigning IDs to individual chunks that indicate the merged chunk it is supposed to belong to. In this dataset, this is done in the field `largerChunk` on chunks. For example, the first chunk you create this way may have the ID 1, the second chunk 2, the third chunk 3 and so on. In this case, all the component chunks of the first complex chunk should have the value `1` for `largerChunk` and so on.
    ...
    ![Figure 26: The same two chunks that need to be merged into a single chunk have been annotated as \`largeChunk = 2\`, indicating that they belong in the same larger chunk.](images/paste-65C42935.png)
...
rez007$trackDF$default = rez007$trackDF$default %>%
    rez_mutate(noCompetitors = countCompetitors(windowSize = 10, between = F,
                                                exclFrag = T))
rez007$trackDF$default = rez007$trackDF$default %>%
  rez_mutate(orderInTrail = getPosInChain(tokenOrder = docWordSeqLast, exclFrag = T))
```
...
pass3_model1 = rez007$trackDF$default %>%
  ...
  lm(charCount ~ noCompetitors + noPrevMentionsIn10 + gapUnitsNew + orderInTrail, .)
summary(pass3_model1)
```

This time, `noCompetitors` is no longer a good predictor, although still close to significance. The coefficient of `gapUnitsNew` is still in the 'wrong' direction, since a larger `gapUnitsNew` still leads to a smaller referential expression, but the effect is not significant any more. The predictor `noPrevMentionsin10` is also insignificant even though the estimate is in the expected direction: The more the number of previous mentions, the shorter the form. The predictor `orderInTrail` is remains significant and in the expected direction - the farther along in the chain, the shorter the form - though the p-value has increased somewhat.
...
![Figure 27: A screenshot of containing a copular clause with the contracted copula *'s*. The referential expression *what I'm gonna tell him* is very long, and is very close to the previous mention, which is just the copular subject.](images/paste-EA1377C0.png){width="530"}
...
pass3_model2 = rez007$trackDF$default %>%
  filter(!is.na(gapUnitsNew), charCount > 0) %>%
  lm(charCount ~ noCompetitors + noPrevMentionsIn10 + gapUnitsNew + orderInTrail + Relation, .)
summary(pass3_model2)
```

As we can see, subjects are likely to be far shorter than non-subjects. The effects of the other factors are almost gone in this model, except `orderInTrail`, which remains significant.
...
![Figure 28: An example of a free NP.](images/paste-F5ED0106.png){alt="Figure 6" width="430"}
...
pass3_model2 = rez007$trackDF$default %>%
  filter(!is.na(gapUnitsNew), charCount > 0) %>%
  lm(charCount ~ noCompetitors + noPrevMentionsIn10 + gapUnitsNew + orderInTrail + Relation + isPredOrFree, .)
summary(pass3_model2)
```
...
pass3_model2 = rez007$trackDF$default %>%
  filter(!is.na(gapUnitsNew), charCount > 0) %>%
  lm(charCount ~ noCompetitors + noPrevSubjMentionsIn10 + noPrevNonSubjMentionsIn10 + gapUnitsNew + orderInTrail + Relation + isPredOrFree, .)
summary(pass3_model2)
```
...
![Figure 29: A screenshot of the .csv file in Excel.](img/sample-excel.PNG){width="100%"}
...
pass3_model3 = rez007$trackDF$default %>%
  filter(!is.na(gapUnitsNew), charCount > 0) %>%
  lm(charCount ~ noCompetitors + noPrevSubjMentionsIn10 + noPrevNonSubjMentionsIn10 + gapUnitsNew + orderInTrail + number + Relation + isPredOrFree, .)
summary(pass3_model3)
```

Whether the current mention is a subject continues to turn out to be a good predictor, as does the order of a mention in the trail. The rest of the predictors are not significant, though some of them (particularly lookback distance, i.e. `gapUnits`) may hold some promise on a larger dataset. Grammatical number seems to be hopeless, and probably not worth pursuing.

## A post-mortem: Where did all the effects go?

It may be disappointing that only two predictors, the order of a track in the trail and the subject/non-subject contrast, were significant. Even though we know from previous studies that previous mentions are generally important in determining the choice of referential form, only the order of a track in the trail turned out to be significant, and other context-related variables did not.
