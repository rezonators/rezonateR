...
## Advanced: Using `getSeqBounds` on other orders
Sometimes, there are other ordered structures in file that you may want to consider, such as prosodic sentences or turns. You may want to get boundary values for these; for example, you might want to know the first and last prosodic sentence numbers inside a stack. At the moment, built-in support for these is limited, but the advanced function `getSeqBounds()` may be used to get upper and lower bounds for these.

TODO: Add an actual example with stacks once stack support is added.
...
Now that you know how to deal with time and sequence, it will be even easier to work with rezonateR. If you're following the whole tutorial series, the next tutorial is `vignette("edit_easyRez")`, where you will start learning to add automatic annotations to `rezrDF`s!
