...
```
To save the imported rezrObj (something you will definitely want to do - nobody wants to import again!), use the function rez_save(). To load it again, use the function rez_load(). Both functions are quite easy: rez_save() simply requires a `rezrObj` and a save path, and `rez_load` only a path to load from.
...
Now that you know your way around .rez files, you can go to the next tutorial to see how time is handled in Rezonator.
