...
In general, I recommend that you copy the `.csv` file before editing it, and edit from the copied file. This is because you don't want to accidentally re-run your code and lose all your edits. In this case, I have added `_edited` to the end of the filename.
...
Finally, the `updateFromDF()` function allows us to update the original rezrDF using information from the new rezrDF. The first argument of this function is the target `rezr`DF` (i.e. the `rezrDF` you're changing), and the second one is the data frame that you're changing from, i.e. the data frame you just imported. The rest of the arguments are all settings:
...
* If you deleted certain elements from Rezonator, simply re-run all your code from the beginning, and ensure that `addRos == FALSE` in `updateFromDF()`.
